...
export default function Home() {
  ...
  const uploadObject = async (file, name) => {
    ...
    try {
      ...
      // setProgress(100);
      // setloading(false);
      takeToMetaverse(avatarUrl);
    } catch (err) {
      console.log("Error Uploading object", err);
      toast.error("Creating Avatar Failed");
      setProgress(99);
      takeToMetaverseWithDefaultAvatar();
  ...
  useEffect(() => {
    if (progress == 100) {
      window.open("https://events.gclverse.com/entrance1/?name=" + name + "&avatarId=" + avatarUrlSplitter(avatarUrl), "_self");
      // setloading(false);
    }
  }, [progress]);

  function takeToMetaverse() {
    window.open("https://events.gclverse.com/entrance1/?name=" + name + "&avatarId=" + avatarUrlSplitter(avatarUrl), "_self");
  }

  function takeToMetaverseWithDefaultAvatar() {
    ...
    window.open("https://events.gclverse.com/entrance1/?name=" + name + "&avatarId=" + newAvatarId, "_self");
  }
      ...
      {loading ? (
        ...
        <>
          <div className="mt-4 ml-2 lg:ml-0 lg:mt-0 lg:absolute lg:top-14 top-4 lg:left-20 left-2 flex lg:items-center justify-between gap-2">
            <Image src={"/logo.svg"} alt="Tech Mahindra Global Chess League" width={150} height={300} className="w-40 sm:w-44" />
          </div>
          ...
          <div className="mx-auto mt-auto sm:mt-10 w-full">
            <h1 className={classNames("hidden sm:block lg:text-3xl text-[22px] text-[#2d1c42]  font-bold text-center mb-8", audiowide.className)}>Welcome to GCL Season 2</h1>
            <div className=" bg-white bg-opacity-30 backdrop-blur-xl lg:px-8 px-4 md:px-8 py-5 text-center  md:max-w-full mx-auto sm:w-[500px] flex flex-col">
              <h1 className={classNames("sm:hidden lg:text-3xl text-[22px] text-[#2d1c42]  font-bold text-center mb-6", audiowide.className)}>Welcome to GCL Season 2</h1>
              <div className="flex mb-2 sm:my-2 sm:hidden gap-x-3 gap-y-2 justify-center flex-wrap ">
                <div className=" flex items-center bg-white py-1 pr-[1.5rem] font-bold text-xs w-fit ">
                  <Image width={400} height={400} className="w-10 h-7" src="/hand.gif" alt="" /> <span className="font-bold"> Total Visitors:</span> <span className="text-[#571ABA] ml-2">{activeVisitors}</span>
                </div>
                <div className="cursor-pointer hidden sm:flex items-center bg-white py-1 px-3 font-bold text-xs w-fit min-w-[125px]" onClick={toggleSound}>
              ...
              <div className="">
                <div className="grid grid-flow-row grid-cols-2   gap-2 ">
                  ...
                  >
                    ...
                    <div>{gender == "male" ? <Checked fill={"#571ABA"} /> : <UnChecked />}</div>
                  </div>
                  ...
                  >
                    ...
                    <div>{gender == "female" ? <Checked fill={"#571ABA"} /> : <UnChecked />}</div>
                  </div>
                </div>
                {gender && <div className="mt-3 font-bold text-[#571ABA]">*&nbsp;Select Your Avatar</div>}
                {/* the avatars */}
                {gender && (
                    ...
                    {gender &&
                      (gender === "male" ? maleAvatars : femaleAvatars).map(({ id, src, height, width, className }) => (
                        <div
                          key={id}
                          onClick={() => {
                            setSelectedAvatarId(src.substring(1, src.length - 5)), setselected(id);
                          }}
                          className={classNames("relative  border-2  overflow-hidden ", selected === id ? "bg-[#F7F644]  border-[#F7F644] " : "bg-white border-black")}
                        >
                          {selected == id && <Image alt="" src={"/check.svg"} className="absolute top-1 left-1 h-5 w-5 z-10" width={200} height={200} />}
                          <Image alt={`look-${id}`} src={src} fill={!height && !width} height={height || undefined} width={width || undefined} className={classNames(className || "", selected === id ? "scale-125 " : "")} />
                        </div>
                      ))}
                  </div>
                )}
              ...
              >
                {enabled ? <SpinnerCircular size={isMobile ? 24 : 28} color="#000" thickness={120} /> : <div onClick={() => setEnabled(true)}>Let&apos;s Visit Friends House in London</div>}
              </button>
              {gender && (
                <div className="flex flex-col gap-2 my-2">
                  ...
                  <div className="mx-auto text-center font-bold text-[1rem] sm:text-xl w-full text-black  ">You can create your own custom look in few {"\n"}simple steps. It is fun! </div>
                  <button onClick={() => setIsFrameOpen(true)} type="button" className={"bg-opacity-100 hover:bg-[#fff700] bg-[#ebff00] border-black border  text-black font-semibold gap-2  py-3 text-center flex items-center w-full justify-center md:text-xl"}>
                    Create My Own Look
                  </button>
                </div>
              )}
              <div className="sm:flex mt-4 my-2 hidden gap-x-3 gap-y-2 justify-center flex-wrap ">
                <div className=" flex items-center bg-white py-1 pr-[1.5rem] font-bold text-xs w-fit ">
                  <Image width={400} height={400} className="w-10 h-7" src="/hand.gif" alt="" /> <span className="font-bold"> Total Visitors:</span>
                  {"   "}
                  <span className="text-[#571ABA] ml-2">{activeVisitors}</span>
                </div>
                <div className=" cursor-pointer hidden sm:flex items-center bg-white py-1 px-3 font-bold text-xs w-fit min-w-[125px]" onClick={toggleSound}>
                  {isSoundOn ? <Image className="w-10 h-5" width={400} height={400} src="/stats.gif" alt="" /> : <Image className="w-10 h-5" width={400} height={400} src="/stats.png" alt="" />}
